...
{
  printf("Status %d, temperatur: %.1f, humidity: %.1f, timestamp: %lf\n", r.status, r.temperature, r.humidity, r.timestamp);
  char database[200];
  sprintf(database,"curl -i -XPOST 'localhost:8086/write?db=mydb' --data-binary 'Temperature,host=1 value=%f' --silent --output /dev/null", r.temperature);
  int status = system(database);
  sprintf(database,"curl -i -XPOST 'localhost:8086/write?db=mydb' --data-binary 'Humidity,host=1 value=%f' --silent --output /dev/null", r.humidity);
